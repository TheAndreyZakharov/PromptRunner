...
const defaults: Settings = { bank_root: "", allowed_languages: ["RU"], active_language: "RU", poll_interval_seconds: 10, send_retry_delay_seconds: 10, generation_timeout_seconds: 600, click_retries: 3, new_chat_every: 100, scroll_after_seconds: 2, min_pause_seconds: 12, ready_confirmations: 2, preserve_clipboard: true, theme: "dark" };
...
const specs = [
  ...
  ["scroll", "Прокрутка ответа", "scroll_answer", "action"],
...
function render() {
  ...
  <main class="shell">
    ...
    <section class="grid ${view === "home" ? "home-grid" : "single-grid"}"><div class="column left-column ${leftVisible ? "" : "empty-column"}">
      ...
      <section class="card ${view === "settings" ? "" : "hidden-panel"}"><div class="card-title"><h2>Настройки ожидания</h2><span class="muted">применяются при старте</span></div><div class="row"><label>Проверка состояния, сек<input id="poll" type="number" min="1" value="${settings.poll_interval_seconds}"></label><label>Тайм-аут генерации, сек<input id="timeout" type="number" min="10" value="${settings.generation_timeout_seconds}"></label></div><div class="row"><label>Повторная отправка через, сек<input id="send-retry-delay" type="number" min="1" max="300" value="${settings.send_retry_delay_seconds}"></label><label>Прокрутка после ответа, сек<input id="scroll-after" type="number" min="0" max="30" value="${settings.scroll_after_seconds}"></label></div><div class="row"><label>Пауза между запросами, сек<input id="cooldown" type="number" min="0" value="${settings.min_pause_seconds}"></label><label>Подтверждений no_prompt<input id="confirmations" type="number" min="1" max="5" value="${settings.ready_confirmations}"></label></div><div class="row"><label>Попыток отправить после вставки<input id="retries" type="number" min="1" max="8" value="${settings.click_retries}"></label><label>Новый чат каждые N вопросов<input id="new-chat" type="number" min="0" value="${settings.new_chat_every}"></label></div><label class="switchline"><input id="preserve-clipboard" type="checkbox" ${settings.preserve_clipboard ? "checked" : ""}> Восстанавливать прежний буфер после записи ответа</label><p class="hint">Если после клика writing не подтверждён, приложение подождёт заданное время и повторит отправку того же промпта. Во время генерации курсор наводится в scroll-зону и прокручивает чат вниз.</p></section>
...
function wire() {
  ...
  if (tab === "action") {
    document.querySelectorAll(".zone-card").forEach((card) => {
      ...
      button.textContent = name === "scroll" ? "Проверить прокрутку" : "Проверить клик";
      button.addEventListener("click", () => { if (!window.confirm(`${name === "scroll" ? "Проверить прокрутку" : "Выполнить тестовый клик"} зоны «${name}»? Убедитесь, что AI-чат открыт.`)) return; invoke("test_action_zone", { zone: zone(name), originX: selectedScreen?.x ?? 0, originY: selectedScreen?.y ?? 0 }).catch((error) => showError(String(error))); });
      card.append(button);
    });
  }
  if (tab === "observation") {
...
async function renderCalibration(requestedScreenIndex?: number) {
  ...
  const defaultSize = (name: string, kind: ZoneKind) => ({
    width: kind === "observation" ? 80 : name === "input" ? 320 : name === "scroll" ? 420 : 20,
    height: kind === "observation" ? 80 : name === "input" ? 80 : name === "scroll" ? 260 : 20,
  });
  ...
  const draw = async (capture: ScreenCapture) => {
    ...
    document.body.innerHTML = '<div class="calibration-toolbar"><label>Монитор<select id="cal-screen">' + available.map((item) => '<option value="' + item.index + '"' + (item.index === capture.screen.index ? " selected" : "") + '>' + (item.is_primary ? "Основной · " : "Монитор · ") + item.width + "×" + item.height + " · дисплей " + (item.index + 1) + '</option>').join("") + '</select></label><label>Масштаб<select id="cal-zoom">' + zoomOptions.map(([value, label]) => '<option value="' + value + '"' + (Math.abs(value - zoom) < 0.001 ? " selected" : "") + '>' + label + '</option>').join("") + '</select></label></div><div class="calibration-palette">' + palette + '<div class="calibration-actions"><input id="cal-profile-name" placeholder="Имя профиля"><span id="calibration-status" class="calibration-status">Калибровка ещё не сохранена</span><button id="calibration-save" class="primary">Готово (' + fixedCount + '/' + specs.length + ')</button><button id="calibration-cancel">Отмена</button></div><span class="palette-hint">Перетащите зоны на снимок, измените размер и зафиксируйте их. Все зоны должны быть зелёными перед сохранением.</span></div><div id="calibration-scroll"><div id="calibration-stage" style="width:' + capture.image_width * zoom + 'px;height:' + capture.image_height * zoom + 'px"><img src="' + capture.data_url + '" width="' + capture.image_width * zoom + '" height="' + capture.image_height * zoom + '" draggable="false" alt="Фон снимка экрана">' + placedItems.map((item) => '<div class="cal-zone ' + item.kind + (fixedZones.has(item.name) ? ' fixed' : '') + '" data-zone="' + item.name + '" aria-label="' + item.label + '" style="left:' + item.x + 'px;top:' + item.y + 'px;width:' + item.width + 'px;height:' + item.height + 'px">' + handles.map((handle) => '<i class="resize-handle handle-' + handle + '" data-handle="' + handle + '" aria-hidden="true"></i>').join('') + '</div>').join("") + "</div></div>";
...
if ((await currentWindow.label) === "calibration") {
  await currentWindow.onCloseRequested(async (event) => { event.preventDefault(); await currentWindow.hide(); });
